...
PACKAGE: Final = "ydcv" if sys.argv[1:2] == [] else sys.argv[1]
...
for action in actions:
    ...
    if "--history" in action.option_strings:
        history_file = action.default
    ...
    if action.choices:
        ...
        metavar = " "
    elif action.metavar == "word":
        # change history_file according to `--history` in zsh_completion.in
        continue
    else:
        completion = ""
...
template = template.replace("{{history_file}}", history_file)
